...
void USB_serial_init(XScuGic intc){
	...
	*usb0_base = 0x00080002; //reset value + reset bit to 1
	...
	status = xusbps_WINUSB_register_interrupt(&intc, &usb, XPAR_PS7_USB_0_INTR);
	if (status != XST_SUCCESS) {
		xil_printf("ERROR: Unable to register USB interrupts: %d\n", status);
		exit(1);
	}

	status = xusb_WINUSB_init(&usb, XPAR_PS7_USB_0_DEVICE_ID, XPAR_PS7_USB_0_INTR, 64 * 1024);
	if (status != XST_SUCCESS) {
...
int USB_serial_available(void){
	return xusb_WINUSB_rx_bytes_available();
}
int USB_serial_Read(u8 *buffer, u32 length){
	return xusb_WINUSB_receive_data(buffer, length);
}
int USB_serial_Write(u8 *buffer, u32 length){
	return xusb_WINUSB_send_data(&usb, buffer, length);
}
int USB_serial_Putc(char c){
	return xusb_WINUSB_send_data(&usb, &c, 1);
}

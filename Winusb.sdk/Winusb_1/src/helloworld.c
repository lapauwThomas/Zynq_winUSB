...
int main(void) {
	...
	XGpioPs_CfgInitialize(&gpio_MIO, ConfigPtrPS,ConfigPtrPS->BaseAddr);
	XGpioPs_SetDirectionPin(&gpio_MIO, 46, 1);
    XGpioPs_SetOutputEnablePin(&gpio_MIO,46, 1);
    XGpioPs_WritePin(&gpio_MIO, 46, 0x0); //assert RESET
    for(uint i = 0; i<100000; i++){ //waste some time
    }
    XGpioPs_WritePin(&gpio_MIO, 46, 0x1); //get PHY out of reset
    for(uint i = 0; i<100000; i++){ //waste some more time
    }
	...
	const char hai[4]="\n\r";
	xil_printf("\n\n--- USB WINUSB ACM Test Application ---\n\n\r");
	...
	xil_printf("setup done\n\r");
		while(1) {
				...
				//bytes = xusb_WINUSB_rx_bytes_available() > 5 ? 5 : xusb_WINUSB_rx_bytes_available();
				bytes = USB_serial_available();
				if (bytes != 0) {
					bytes = USB_serial_Read(text_buffer, bytes);
					text_buffer[bytes] = 0;
